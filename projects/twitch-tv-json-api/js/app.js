(() => {
  const main = function _main() {
    ...
    const handleData = function _handleData() {
      const template = `
      <div class="channel">
        <img src="" alt="" id="channel-photo">
        <span id="status-light"></span>
        <a class="channel-info" href="" target="_blank"></a>
        <br>
        <span id="online-status" class="channel-info"></span>
        <br>
        <span id="current-subject"></span>
      </div>`;

      // Create div for each object in the data
      data.forEach(obj => {
        if (obj.error)
          return;
        $('main').append(template);
          const target = $('main > .channel').last();
          const
            img = target.find('img'),
            statusLight = target.find('span#status-light'),
            channelLink = target.find('a'),
            status = target.find('span#online-status'),
            currentSubject = target.find('span#current-subject');


          // Original data didn't allow me to add profile photo for
          // offline streamers so I had to add
          if (obj.stream) {
            img.prop('src', obj.stream.profile_banner);
            ...
            channelLink.text(obj.display_name);
            status.text('offline');
          }

          channelLink.prop('href', 'https://twitch.tv/' + channelLink.text());
      });
    };
  ...
  const data = [
    ...
    },
    {
      ...
      "profile_banner": "https://static-cdn.jtvnw.net/jtv_user_pictures/ogamingsc2-profile_image-9021dccf9399929e-70x70.jpeg",
      "_links": {
    ...
    },
    {
      ...
      "profile_banner": "https://static-cdn.jtvnw.net/jtv_user_pictures/noobs2ninjas-profile_image-34707f847a73d934-70x70.png",
      "_links": {
        "self": "https://api.twitch.tv/kraken/streams/esl_sc2",
        "channel": "https://api.twitch.tv/kraken/channels/esl_sc2"
      }
